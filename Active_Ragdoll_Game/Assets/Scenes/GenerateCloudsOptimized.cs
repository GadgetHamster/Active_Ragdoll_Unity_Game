...
public class GenerateCloudsOptimized : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int cloudY = 0; cloudY < m_cloudsCount; cloudY++)
        {
            for (int cloudX = 0; cloudX < m_cloudsCount; cloudX++)
            {
                ...
                {
                    x = offset * CloudSize + transform.position.x + cloudX * m_cloudSize * CloudSize,
                    y = transform.position.y,
                    z = offset * CloudSize + transform.position.z + cloudY * m_cloudSize * CloudSize,
                };
    ...
    private void UpdateBatch(object input)
    {
        ...
        for (int cloudIndex = 0; cloudIndex < m_batches[frameParams.BatchIndex].Length; cloudIndex++)
        {
            ...
            m_batches[i].Objects[j].m00 = scale * CloudSize;
            m_batches[i].Objects[j].m11 = scale * CloudSize;
            m_batches[i].Objects[j].m22 = scale * CloudSize;
        }
    }
